            ...
            <div class="card bg-secondary shadow">
                <div class="card-header bg-white border-0">
                    <div class="row align-items-center">
                        <h3 class="mb-0">{{ __('Tambah Data Pegawai') }}</h3>
                    </div>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ route('simpanpegawai') }}" autocomplete="off">
                        {{ csrf_field () }}
                        <div class="pl-lg-4">
                            <div class="form-group">
                                <label class="form-control-label" for="input-name">{{ __('Nama') }}</label>
                                <input type="text" name="nama" id="nama" class="form-control"
                                    placeholder="{{ __('Masukan Nama Pegawai') }}">
                            </div>
                            <div class="form-group">
                                <label class="form-control-label" for="input-name">{{ __('Alamat') }}</label>
                                <textarea name="alamat" id="alamat" class="form-control"
                                    placeholder="{{ __('Masukan Alamat Pegawai') }}"></textarea>
                            </div>
                            <div class="form-group">
